...
namespace AssetStudio
{
    ...
    public class ShaderProgram
    {
        ...
        {
            int subProgramsCapacity = reader.ReadInt32();
    ...
    public class ShaderSubProgram
    {
        ...
        public ShaderSubProgram(BinaryReader reader)
        {
            ...
            if (this.magic >= 201608170) //5.5.0 and up
            {
                reader.BaseStream.Position += 4;
            }
            int keywordCount = reader.ReadInt32();
            this.m_Keywords = new string[keywordCount];
            for (var i = 0; i < keywordCount; i++)
            {
                this.m_Keywords[i] = reader.ReadAlignedString();
            }
            this.m_ProgramCode = reader.ReadBytes(reader.ReadInt32());
            ...
            if (this.m_ProgramCode.Length > 0)
            {
                switch (this.m_ProgramType)
                {
                    ...
                    case ShaderGpuProgramType.kShaderGpuProgramGLCore43:
                        sb.Append(Encoding.UTF8.GetString(this.m_ProgramCode));
                        break;
                    ...
                    {
                        var shaderBytecode = new ShaderBytecode(this.m_ProgramCode);
                    ...
                    {
                        var start = 6;
                        if (this.magic == 201509030) // 5.3
                        {
                            start = 5;
                        }
                        var buff = new byte[this.m_ProgramCode.Length - start];
                        Buffer.BlockCopy(this.m_ProgramCode, start, buff, 0, buff.Length);
                    ...
                    case ShaderGpuProgramType.kShaderGpuProgramMetalFS:
                        using (var reader = new BinaryReader(new MemoryStream(this.m_ProgramCode)))
                        {
                            ...
                            string entryName = reader.ReadStringToNull();
                            byte[] buff = reader.ReadBytes((int) (reader.BaseStream.Length - reader.BaseStream.Position));
                            sb.Append(Encoding.UTF8.GetString(buff));
                        }
                        break;
                    default:
                        sb.Append($"/*Unsupported program data {this.m_ProgramType}*/");
                        break;
                }
            }
            sb.Append('"');
